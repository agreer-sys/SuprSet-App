...
export interface BlockParams {
  ...
  // pacing (rep-rounds)
  pacingScript?: { tStartSec:number; tEndSec:number; exerciseId:string }[];
  rounds?: number;
  roundRestSec?: number;            // 0 for nonstop
}
...
export interface TimelineContext {
  ...
  // Beep kinds used app-side: 'countdown' (short), 'start' (GO), 'end' (finish)
  beep?: (kind:'countdown'|'start'|'end') => void;
